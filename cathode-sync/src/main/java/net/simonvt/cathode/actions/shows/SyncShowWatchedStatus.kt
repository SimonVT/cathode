...
import net.simonvt.cathode.actions.shows.SyncShowWatchedStatus.Params
...
class SyncShowWatchedStatus @Inject constructor(
  ...
  override fun key(params: Params): String = "SyncShowWatchedStatus&traktId=${params.traktId}"

  override fun getCall(params: Params): Call<ShowProgress> =
    showsService.getWatchedProgress(params.traktId)
    ...
    for (season in response.seasons) {
      ...
      for (episode in season.episodes) {
        ...
        val builder = ContentProviderOperation.newUpdate(Episodes.withId(episodeId))
          .withValue(EpisodeColumns.WATCHED, episode.completed)
          .withValue(EpisodeColumns.LAST_WATCHED_AT, episode.last_watched_at?.timeInMillis)
