...
import net.simonvt.cathode.actions.user.SyncUserSeasonComments.Params
...
class SyncUserSeasonComments @Inject constructor(
  ...
  override fun key(params: Params): String = "SyncUserSeasonComments"

  override fun getCall(params: Params, page: Int): Call<List<CommentItem>> =
    usersService.getUserComments(CommentType.ALL, ItemTypes.SEASONS, page, LIMIT)
  ...
  override suspend fun handleResponse(params: Params, page: Int, response: List<CommentItem>) {
    ...
    val localComments = context.contentResolver.query(
      ...
      CommentColumns.ITEM_TYPE + "=" + DatabaseContract.ItemType.SEASON + " AND " + CommentColumns.IS_USER_COMMENT + "=1"
    )
    ...
    if (params.userActivityTime > 0L) {
      TraktTimestamps.getSettings(context)
        .edit()
        .putLong(TraktTimestamps.SEASON_COMMENT, params.userActivityTime)
        .apply()
    }
  }
