...
import net.simonvt.cathode.actions.user.SyncUserShowComments.Params
...
class SyncUserShowComments @Inject constructor(
  ...
  override fun key(params: Params): String = "SyncUserShowComments"

  override fun getCall(params: Params, page: Int): Call<List<CommentItem>> =
    usersService.getUserComments(CommentType.ALL, ItemTypes.SHOWS, page, LIMIT)
  ...
  override suspend fun handleResponse(params: Params, page: Int, response: List<CommentItem>) {
    ...
    val localComments = context.contentResolver.query(
      ...
      CommentColumns.ITEM_TYPE + "=" + ItemTypeString.SHOW + " AND " + CommentColumns.IS_USER_COMMENT + "=1"
    )
    ...
    for (commentItem in response) {
      ...
      val result = showHelper.getIdOrCreate(traktId)
      val showId = result.showId

      values.put(CommentColumns.ITEM_TYPE, ItemType.SHOW.toString())
    ...
    if (params.userActivityTime > 0L) {
      TraktTimestamps.getSettings(context)
        .edit()
        .putLong(TraktTimestamps.SHOW_COMMENT, params.userActivityTime)
        .apply()
    }
  }
