...
public class SyncHiddenCollected extends PagedCallJob<HiddenItem> {
  ...
  public SyncHiddenCollected() {
    super(Flags.REQUIRES_AUTH);
  }

  @Override public String key() {
    return "SyncHiddenCollected";
  }
  ...
  @Override public Call<List<HiddenItem>> getCall(int page) {
    return usersService.getHiddenItems(HiddenSection.PROGRESS_COLLECTED, null, page, 25);
  }
  ...
  @Override public boolean handleResponse(List<HiddenItem> items) {
    ...
    Cursor hiddenShows = getContentResolver().query(Shows.SHOWS, new String[] {
        ShowColumns.ID,
    }, ShowColumns.HIDDEN_COLLECTED + "=1", null, null);
    ...
    Cursor hiddenSeasons = getContentResolver().query(Seasons.SEASONS, new String[] {
        SeasonColumns.ID,
    }, SeasonColumns.HIDDEN_COLLECTED + "=1", null, null);
