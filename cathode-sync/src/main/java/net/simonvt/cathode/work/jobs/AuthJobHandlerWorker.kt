...
class AuthJobHandlerWorker @AssistedInject constructor(
  @Assisted val context: Context,
  @Assisted params: WorkerParameters,
  private val authJobHandler: AuthJobHandler
  ...
  override suspend fun doWork(): Result {
    if (authJobHandler.hasJobs()) {
      suspendCancellableCoroutine<Unit> { cont ->
        ...
        cont.invokeOnCancellation { authJobHandler.unregisterListener(listener) }
  ...
  companion object {
    const val TAG = "AuthJobHandlerWorker"
    const val TAG_DAILY = "AuthJobHandlerWorker_daily"
  }
}
