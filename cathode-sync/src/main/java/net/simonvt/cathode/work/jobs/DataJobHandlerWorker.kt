...
class DataJobHandlerWorker @AssistedInject constructor(
  @Assisted val context: Context,
  @Assisted params: WorkerParameters,
  private val dataJobHandler: DataJobHandler
  ...
  override suspend fun doWork(): Result {
    if (dataJobHandler.hasJobs()) {
      suspendCancellableCoroutine<Unit> { cont ->
        ...
        cont.invokeOnCancellation { dataJobHandler.unregisterListener(listener) }
        dataJobHandler.registerListener(listener)
      }
    }
  ...
  companion object {
    const val TAG = "DataJobHandlerWorker"
    const val TAG_DAILY = "DataJobHandlerWorker_daily"
  }
}
