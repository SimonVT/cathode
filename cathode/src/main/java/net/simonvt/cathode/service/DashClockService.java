...
public class DashClockService extends DashClockExtension {
  ...
  protected void onUpdateData(int reason) {
    ...
    Cursor c = getContentResolver().query(CathodeContract.Episodes.CONTENT_URI, null,
        CathodeContract.Episodes.FIRST_AIRED + ">? AND " + CathodeContract.Episodes.WATCHED + "=0",
        new String[] {
    ...
    if (c.moveToFirst()) {
      ...
      Cursor show =
          getContentResolver().query(CathodeContract.Shows.buildShowUri(showId), null, null, null,
              null);
      if (!show.moveToFirst()) {
        show.close();
        return; // Wat
