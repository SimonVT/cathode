...
public final class StaggeredGridAnimator {
  ...
  public interface Callback {
     ...
     *
     * @param parent The StaggeredGridView where the view is being added.
     ...
     */
    boolean onAddView(StaggeredGridView parent, View view, int position, long id);

    /**
     * Use to handle animating adding a view.
     *
     * @param parent The StaggeredGridView where the view is being moved.
     ...
     * @return True if a custom animation has been started, false to use default.
     */
    boolean onMoveView(StaggeredGridView parent, View view, int position, long id, Rect startBounds,
        Runnable endAction);

    /**
     * Use to handle animating adding a view.
     *
     * @param parent The StaggeredGridView where the view is being removed.
     * @param view The view that is being removed.
     * @param id The id of the item the view represents.
     * @param startBounds The bounds of the view before the dataset changed.
     * @return True if a custom animation has been started, false to use default.
     */
    boolean onRemoveView(StaggeredGridView parent, View view, long id, Rect startBounds);
  }
  ...
  public StaggeredGridAnimator(StaggeredGridView staggeredGridView) {
    this(staggeredGridView, null);
  }

  public StaggeredGridAnimator(StaggeredGridView staggeredGridView, Callback callback) {
  ...
  private void beforeDataSetChanged() {
    ...
    Adapter adapter = staggeredGridView.getAdapter();
    final int firstVisiblePosition = staggeredGridView.getFirstPosition();
    for (int i = 0, childCount = staggeredGridView.getChildCount(); i < childCount; i++) {
    ...
    observer.addOnPreDrawListener(new ViewTreeObserver.OnPreDrawListener() {
      @Override public boolean onPreDraw() {
        ...
        for (int i = 0, childCount = staggeredGridView.getChildCount(); i < childCount; i++) {
          ...
          if (bounds != null) {
            if (callback == null || !callback.onMoveView(staggeredGridView, child, position, id,
                bounds, endAction)) {
            ...
            if (callback == null || !callback.onAddView(staggeredGridView, child, position, id)) {
              child.setAlpha(0.0f);
        ...
        for (int i = 0; i < size; i++) {
          ...
          if (overlay == null) {
            ViewGroup parent = (ViewGroup) staggeredGridView.getParent();
            overlay = parent.getOverlay();
            staggeredGridView.getLocationOnScreen(gridLocation);
            parent.getLocationOnScreen(hostViewLocation);
          }

          overlay.add(viewCopy);
          viewCopy.offsetLeftAndRight(gridLocation[0] - hostViewLocation[0]);
          viewCopy.offsetTopAndBottom(gridLocation[1] - hostViewLocation[1]);

          if (callback == null || !callback.onRemoveView(staggeredGridView, viewCopy, id, bounds)) {
            viewCopy.animate()
