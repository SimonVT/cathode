...
public class DashClockService extends DashClockExtension {
  ...
  protected void onUpdateData(int reason) {
    ...
    Cursor c = getContentResolver().query(TraktContract.Episodes.CONTENT_URI, null,
        TraktContract.Episodes.FIRST_AIRED + ">? AND " + TraktContract.Episodes.WATCHED + "=0",
        new String[] {
    ...
    if (c.moveToFirst()) {
      ...
      Cursor show =
          getContentResolver().query(TraktContract.Shows.buildShowUri(showId), null, null, null,
              null);
      if (!show.moveToFirst()) {
        show.close();
        return; // Wat
