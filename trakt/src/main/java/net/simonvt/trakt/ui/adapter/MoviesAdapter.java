...
public class MoviesAdapter extends CursorAdapter {
  ...
  public void bindView(View view, Context context, Cursor cursor) {
    ...
    final long id = cursor.getLong(cursor.getColumnIndex(TraktContract.Movies._ID));
    final boolean watched = cursor.getInt(cursor.getColumnIndex(TraktContract.Movies.WATCHED)) == 1;
    final boolean inCollection =
        cursor.getInt(cursor.getColumnIndex(TraktContract.Movies.IN_COLLECTION)) == 1;
    final boolean inWatchlist =
        cursor.getInt(cursor.getColumnIndex(TraktContract.Movies.IN_WATCHLIST)) == 1;

    vh.poster.setImage(cursor.getString(cursor.getColumnIndex(TraktContract.Movies.POSTER)));
    vh.title.setText(cursor.getString(cursor.getColumnIndex(TraktContract.Movies.TITLE)));
    vh.overview.setText(cursor.getString(cursor.getColumnIndex(TraktContract.Movies.OVERVIEW)));
